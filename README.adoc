...
The source for each helper package lives under `helper-packages/` (installed to `/var/lib/pi-gen-micro/helper-packages/`). Each is a directory containing an `nfpm.yaml` and the files to package.

=== Automatic rebuilding

When `pi-gen-micro` runs, it checks whether `nfpm` is installed. If so, it automatically rebuilds all helper packages from source and regenerates the package index before the build starts. This means edits to helper package sources take effect on the next `pi-gen-micro` run with no manual steps.

If `nfpm` is not installed, the pre-built `.deb` files in `internal/packages/` are used as-is.

=== Included helper packages

Each helper package has a full description in its `nfpm.yaml` file. To list all available packages with their descriptions:
...
2. Add the files to be packaged and an `nfpm.yaml` describing the package. See https://nfpm.goreleaser.com/configuration for the full nfpm configuration reference.
...
```
4. Reference the package name (from `nfpm.yaml`, not the directory name) in your configuration's `packages.list`.

The package will be built automatically on the next `pi-gen-micro` run.

=== Adding external pre-built packages

To include a `.deb` not built from an nfpm source (e.g. `rpi-fastbootd`, `rpi-imager-embedded`):
...
NOTE: `build-packages` does not touch `.deb` files that lack a corresponding nfpm source, so externally-provided packages are preserved across rebuilds.

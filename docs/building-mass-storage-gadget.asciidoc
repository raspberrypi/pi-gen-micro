...
`mass-storage-gadget` is created by installing a debian package into the build. The debian package has been prebuilt for you, and can be found in the `internal/packages/` folder. To direct `pi-gen-micro`  to consume this package into the build, we will click on the `add .deb files` button. 
...
Here we want to enter `internal/packages/mass-storage-gadget_1.0.0_arm64.deb`. To add the package to be staged, click `add`. You should now observe that in the `Additional Components` table, that a new entry has appeared: `"DEBIAN: internal/packages/mass-storage-gadget_1.0.0_arm64.deb"`. This indicates that `pi-gen-micro`  will take the debian package and install that into the build image.
...
You will also need to insert your public SSH key into `internal/prebuilts/authorized_keys`, which `pi-gen-micro` will copy into the image.
